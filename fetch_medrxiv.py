...
pdf_dir = Path('data/medrxiv/pdfs')
texts_dir = Path('data/medrxiv/texts')
...
def fetch_by_month(mdb, start_m, end_m, limit=None):
    try:
        ...
        finished = False
        while not finished:
            url = f"https://api.medrxiv.org/details/medrxiv/{start_d}/{end_d}/{offset}"
...
def fetchmedrxiv(ctx, limit):
    month_ranges = ctx.obj['month_ranges']
    for month_range in month_ranges:
        start_m, end_m = month_range.split(':')
        fetch_by_month(ctx.obj['mdb_medrxiv']['papers'], start_m, end_m, limit)


def download_papers(ctx, start_m, pdf_dir, limit=None):
    paper_info_collection = ctx.obj['mdb_medrxiv']['papers']
...
def integrate(ctx, limit):
    ...
    with open(f'data/medrxiv/fulltext_info_{month_suffix}.json', 'w') as f:
